...
@CapabilityDescription("Retrieves Change Data Capture (CDC) events from a PostgreSQL database, using wal2json output plugin. wal2json MUST support \"format-version\" option. Works for PostgreSQL version 9.6. CDC Events include INSERT, UPDATE, DELETE operations. Events "
...
public class Wal2JsonPostgreSQL96Capture extends AbstractProcessor {
    // Relationships
    ...
    public static final PropertyDescriptor SCHEMA_NAME = new PropertyDescriptor.Builder().name("cdc-postgresql-schema-name").displayName("Schema")
            .description("PostgreSQL schema name. Schema defines tables whose data changes are intended to be replicated through logical replication.")
            .required(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR).build();

    public static final PropertyDescriptor TABLES = new PropertyDescriptor.Builder().name("cdc-postgresql-tables").displayName("Tables")
            .description("include only rows from the specified tables. Default is all tables from all schemas.")
            .required(true)
            .addValidator(StandardValidators.createListValidator(true, true, StandardValidators.NON_EMPTY_VALIDATOR)).build();
    ...
    public void setup(ProcessContext context) {
        ...
        //adds schemaname for each tableName
        final String tables = Arrays.asList(context.getProperty(TABLES).evaluateAttributeExpressions().getValue().split(",")).stream()
                .map(t -> schemaName.concat(".").concat(t.trim()))
                .reduce((x, y) -> x + "," + y).get();
        ...
        try {
            ...
            this.pgEasyReplication = new PGEasyReplication(tables, schemaName, replicationSlotName, this.connectionManager);
